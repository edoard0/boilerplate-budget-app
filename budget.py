class Category:
    ...
    def __repr__(self):
      ...
      item_list=[self.ledger[i]["description"][:23]+str(('%7.2f' % self.ledger[i]["amount"])[:7]).rjust(30 - len(self.ledger[i]["description"])) for i in range(len(self.ledger))] 
      item_list_str=("\n").join(item_list)+"\n"
...
def create_spend_chart(categories):
  ...
  ratios=[int((i.total_withdrawals()/total_balance*100)/10)*10 for i in categories]
  ...
  words_list_padded=[word+" "*(max_len-len(word)) for word in words_list]
